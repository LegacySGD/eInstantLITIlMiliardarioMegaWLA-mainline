			...
			<lxslt:component prefix="my-ext" functions="formatJson,retrievePrizeTable,getType">
				<lxslt:script lang="javascript">
					...
					function formatJson(jsonContext, translations, prizeTable, prizeValues, prizeNamesDesc)
					{
						...
						const gridCols 		= 5;
						const gridRows 		= 4;
						const winNamesStr   = ["lingotto", "moneta", "ferro", "stella"];
						const prizeNamesStr = ["winAll", "currency", "horseshoe", "star"];

						var winType = [0,0,0];
						var jollyNum = scenarioWinningNumbers[5];
						var r = [];
						...
						function showYourNums(A_strCanvasId, A_strCanvasElement, A_arrGrid)
						{
							...
							for (var i = 0; i < A_arrGrid.length-1; i++)
							{
								if (A_arrGrid[i].slice(1, 9) == winNamesStr[0])
								{
									winAll = true;
								}
							}
							...
							for (var gridRow = 0; gridRow < gridRows; gridRow++)
							{
								for (var gridCol = 0; gridCol < gridCols; gridCol++)
								{
									...
									symbCell = A_arrGrid[tempNum].split(":")[0];
									prizeCell = A_arrGrid[tempNum].split(":")[1];
									prizeStr = convertedPrizeValues[getPrizeNameIndex(prizeNames, prizeCell)];

									boolWinCell = false;
									boolWinCell = (symbCell[0] == '*');
									boolJolly = (symbCell == jollyNum);
									symbCell = boolWinCell ? symbCell.slice(1) : symbCell;
				
									boxColourStr  = (winAll == true) ? colourLime : ((boolWinCell == true) ? colourLime : colourWhite);
						...
						r.push('</td>');
						r.push('<td colspan="1">');
						r.push(getTranslationByName("jokerNumber", translations));
						r.push('</td>');
						r.push('</tr>');
